...
```ts
  ...
  48 |         await filterLocator.waitFor(10000); //waiting for the filter to be applied and products to be sorted
  49 |         isSortedCheck(filterLocator,copyProductName,copyPrice);
  50 |     }
  51 |     async function isSortedCheck(arr, copyProductName, copyPrice) {
