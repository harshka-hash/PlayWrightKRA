...
- Name: ProductFilter.spec.js >> Product Filter Functionality >> TC_04: Verify Product sorting ascending to descending
- Location: tests/ProductFilter.spec.js:12:5
...
```ts
  ...
  51 |         const value = await arr.inputValue();
  52 |         if(value === "az"){
> 53 |             await arr.waitFor({ state: "visible", timeout: 10000 });// To check if the products are sorted in ascending order
     |                       ^ Error: locator.waitFor: Test ended.
